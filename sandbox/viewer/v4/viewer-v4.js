...
class EnhancedViewer {
    ...
    /**
     * Resolve the image list for an article.
     * Priority order:
     *   1. <div class="viewer-images"> containing <img> or data-src items
     *   2. data-images="url1, url2, url3" attribute on the article
     *   3. Single background-image from the article's style attribute
     */
    getImages(article) {
        // 1. Explicit image list element
        const imageListEl = article.querySelector('.viewer-images');
        if (imageListEl) {
            const items = imageListEl.querySelectorAll('[data-src], img');
            if (items.length) {
                return Array.from(items).map(el => el.dataset.src || el.src).filter(Boolean);
            }
        }

        // 2. data-images attribute
        const dataImages = article.getAttribute('data-images');
        if (dataImages) {
            const list = dataImages.split(',').map(s => s.trim()).filter(Boolean);
            if (list.length) return list;
        }

        // 3. Fallback: single tile background-image
...
document.addEventListener('DOMContentLoaded', () => {
    console.log('Initializing EnhancedViewer v3...');
