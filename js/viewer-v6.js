...
//  [x] Side dots start visible, fade to low opacity, show 100% on interact
//  [x] Collect viewer CSS patches, remove !important uses
//  [ ] Add close to `viewer-content-area` for horizontal + vertical
//  [ ] Fix how labels come over from tile to `viewer-content-area` in the description.
//  [ ] Future: data-set-id maps to API / sets.json for dynamic hydration
//
//  REF: https://claude.ai/chat/4319177f-0ba2-43bb-bf5b-752c4b1d9507
//
...
class EnhancedViewer {
    ...
    goToImage(index) {
        ...
        // Update arrow boundary state (dim at first/last — CSS handles opacity via class)
        ...
        if (prev) prev.classList.toggle('is-boundary', index === 0);
        if (next) next.classList.toggle('is-boundary', index === this.images.length - 1);
